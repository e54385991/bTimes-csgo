...
public OnAllPluginsLoaded()
{
	if(hMsgStart == INVALID_HANDLE || hMsgText == INVALID_HANDLE || hMsgVar == INVALID_HANDLE)
	{
		...
		{
			hMsgStart = CreateConVar("ssj_msgstart", "{grey}[ {green}SSJ {grey}] - ", "SSJ messages prefix.");
			hMsgText = CreateConVar("ssj_msgtext", "{grey}", "SSJ messages color.");
			hMsgVar = CreateConVar("ssj_msgvar", "{blue}", "SSJ variables color.");
		}
	}
...
SSJ_PrintStats(client, target)
{
	...
	if((g_iUsageMode[client] == USAGE_SIXTH && g_iJump[target] == 6) || (g_iUsageMode[client] == USAGE_EVERY_SIXTH && !(g_iJump[client] % 6)))
	{
		...
		if(!g_bCSGO)
			PrintColorChat(client, SSJText);
		else CPrintToChat(client, SSJText);
	}
...
PrintColorChat(client, String:msg[], any:...)
{
