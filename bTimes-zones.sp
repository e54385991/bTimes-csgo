...
{	
	...
	g_hZoneOffset[MAIN_START]  = CreateConVar("timer_mainstart_offset", "100", "Set the the default height for the main start zone.");
	g_hZoneOffset[MAIN_END]    = CreateConVar("timer_mainend_offset", "100", "Set the the default height for the main end zone.");
	g_hZoneOffset[BONUS_START] = CreateConVar("timer_bonusstart_offset", "100", "Set the the default height for the bonus start zone.");
	g_hZoneOffset[BONUS_END]   = CreateConVar("timer_bonusend_offset", "100", "Set the the default height for the bonus end zone.");
...
UpdateZoneBeamTexture(Zone)
{
	if(g_GameType == GameType_CSS)
	{
		g_Properties[Zone][ModelIndex] = PrecacheModel("materials/sprites/trails/bluelightning");
	}
	else if(g_GameType == GameType_CSGO)
	{
...
DrawZone(Zone, ZoneNumber, Float:life)
{
	...
	for(new i=0, i2=3; i2>=0; i+=i2--)
	{
		for(new j=1; j<=7; j+=(j/2)+1)
		{
			if(j != 7-i)
			{
				TE_SetupBeamPoints(g_Zones[Zone][ZoneNumber][i], g_Zones[Zone][ZoneNumber][j], g_Properties[Zone][ModelIndex], g_Properties[Zone][HaloIndex], 0, 0, (life < 0.1)?0.1:life, 1.0, 1.0, 10, 0.0, color, 0);
