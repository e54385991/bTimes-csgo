...
public OnPluginStart()
{
	...
	// Cvars
	g_hZoneWidth 			  = CreateConVar("timer_zonewidth", "1.0", "Set the the default width for the all map zones.", 0, true, 0.1, true, 10.0);
	...
	g_hZoneTexture[MAIN_START]  = CreateConVar("timer_mainstart_tex", "materials/effects/blueblacklargebeam", "Texture for main start zone. (Exclude the file types like .vmt/.vtf)");
	g_hZoneTexture[MAIN_END]    = CreateConVar("timer_mainend_tex", "materials/effects/blueblacklargebeam", "Texture for main end zone.");
	g_hZoneTexture[BONUS_START] = CreateConVar("timer_bonusstart_tex", "materials/effects/blueblacklargebeam", "Texture for bonus start zone.");
	g_hZoneTexture[BONUS_END]   = CreateConVar("timer_bonusend_tex", "materials/effects/blueblacklargebeam", "Texture for main end zone.");
	g_hZoneTexture[ANTICHEAT]   = CreateConVar("timer_ac_tex", "materials/effects/blueblacklargebeam", "Texture for anti-cheat zone.");
	g_hZoneTexture[FREESTYLE]   = CreateConVar("timer_fs_tex", "materials/effects/blueblacklargebeam", "Texture for freestyle zone.");
...
public Action:SM_R(int client, int args)
{
	...
	{
		CPrintToChat(client, "%s%sThe main start zone is not ready yet.",
			g_msg_start,
			g_msg_textcol);
	}
...
public Action:SM_End(int client, int args)
{
	...
	{
		CPrintToChat(client, "%s%sThe main end zone is not ready yet.",
			g_msg_start,
			g_msg_textcol);
	}
...
public Action:SM_B(int client, int args)
{
	...
	{
		CPrintToChat(client, "%s%sThe bonus zone has not been created.",
			g_msg_start,
			g_msg_textcol);
	}
...
public Action:SM_EndB(int client, int args)
{
	...
	{
		CPrintToChat(client, "%s%sThe bonus end zone has not been created.",
			g_msg_start,
			g_msg_textcol);
	}
...
CreateZone(client, Zone)
{
	if(ClientCanCreateZone(client, Zone))
	{
		if((g_Properties[Zone][Count] < g_Properties[Zone][Max]) || g_Properties[Zone][Replaceable] == true)
		{
			...
			{
				CPrintToChat(client, "%s%sYou are already setting up a different zone (%s%s%s).",
					g_msg_start,
					g_msg_textcol,
					g_msg_varcol,
		...
		{
			CPrintToChat(client, "%s%sThere are too many of this zone (Max %s%d%s).",
				g_msg_start,
				g_msg_textcol,
				g_msg_varcol,
	...
	{
		CPrintToChat(client, "%s%sSomeone else is already creating this zone (%s%s%s).",
			g_msg_start,
			g_msg_textcol,
			g_msg_varcol,
...
DrawZone(Zone, ZoneNumber, Float:life)
{
	...
	for(new i=0, i2=3; i2>=0; i+=i2--)
	{
		for(new j=1; j<=7; j+=(j/2)+1)
		{
			if(j != 7-i)
			{
				TE_SetupBeamPoints(g_Zones[Zone][ZoneNumber][i], g_Zones[Zone][ZoneNumber][j], g_Properties[Zone][ModelIndex], g_Properties[Zone][HaloIndex], 0, 0, (life < 0.1)?0.1:life, GetConVarFloat(g_hZoneWidth), GetConVarFloat(g_hZoneWidth), 10, 0.0, color, 0);
					...
					switch(Zone)
					{
						...
						case ANTICHEAT:
						{
							if(IsBeingTimed(other, TIMER_MAIN) && (g_Properties[Zone][Flags][ZoneNumber] & FLAG_ANTICHEAT_MAIN))
							{
								...
								CPrintToChat(other, "%s%sYour timer was stopped for using a shortcut.",
									g_msg_start,
									g_msg_textcol);
							}
...
DB_DeleteZone(client, Zone, ZoneNumber, bool:ManualDelete = false)
{
			...
			if(ManualDelete == true)
			{
				if(Zone == MAIN_START || Zone == MAIN_END)
				{
					if(IsBeingTimed(client2, TIMER_MAIN))
					{
						...
						CPrintToChat(client2, "%s%sYour timer was stopped because the %s%s%s zone was deleted.",
							g_msg_start,
							g_msg_textcol,
							g_msg_varcol,
							g_Properties[Zone][Name],
	...
	{
		CPrintToChat(client, "%s%sAttempted to delete a zone that doesn't exist.",
			g_msg_start,
			g_msg_textcol);
	}
}
						...
						switch(Zone)
						{
							...
							case ANTICHEAT:
							{
								if(IsBeingTimed(client, TIMER_MAIN) && g_Properties[Zone][Flags][ZoneNumber] & FLAG_ANTICHEAT_MAIN)
								{
									...
									CPrintToChat(client, "%s%sYour timer was stopped for using a shortcut.",
										g_msg_start,
										g_msg_textcol);
								}
