		...
		if(g_ClientUseCustom[client] & CC_HASCC)
		{
			...
			if(args == 0)
			{
				...
				if(g_ClientUseCustom[client] & CC_MSGCOL)
					CPrintToChat(client, "%s%sColored message enabled.",
						g_msg_start,
						g_msg_textcol);
				else
					CPrintToChat(client, "%s%sColored message disabled.",
						g_msg_start,
						g_msg_textcol);
			}
			else
			{
				...
				CPrintToChat(client, "%s%sColored message set to %s%s",
					g_msg_start,
					g_msg_textcol,
					g_msg_varcol,
					sArg);
...
public Action:SM_Colorhelp(client, args)
{
	...
	{
		CPrintToChat(client, "%s%sLook in console for help with custom color chat.",
			g_msg_start,
			g_msg_textcol);
	}
...
public Action:SM_ReloadRanks(client, args)
{
	...
	CPrintToChat(client, "%s%sChat ranks reloaded.",
		g_msg_start,
		g_msg_textcol);
...
public EnableCC_Callback1(Handle:owner, Handle:hndl, String:error[], any:pack)
{
	if(hndl != INVALID_HANDLE)
	{
		...
		if(SQL_GetRowCount(hndl) > 0)
		{
			...
			if(!(ccuse & CC_HASCC))
			{
				CPrintToChat(client, "%s%sA player with the name '%s%s%s' <%s%s%s> will be given custom chat privileges.",
					g_msg_start,
					g_msg_textcol,
					g_msg_varcol,
					sName,
			...
			{
				CPrintToChat(client, "%s%sA player with the given SteamID '%s%s%s' (name '%s%s%s') already has custom chat privileges.",
					g_msg_start,
					g_msg_textcol,
					g_msg_varcol,
					sAuth,
		...
		{
			CPrintToChat(client, "%s%sNo player in the database found with '%s%s%s' as their SteamID.",
				g_msg_start,
				g_msg_textcol,
				g_msg_varcol,
				sAuth,
	...
	for(new client = 1; client <= MaxClients; client++)
	{
		if(IsClientInGame(client))
		{
			...
			if(StrEqual(sAuth, sAuth2))
			{
				...
				CPrintToChat(client, "%s%sYou have been given custom chat privileges. Type sm_cchelp or ask for help to learn how to use it.",
					g_msg_start,
					g_msg_textcol);
...
public DisableCC_Callback1(Handle:owner, Handle:hndl, String:error[], any:pack)
{
	if(hndl != INVALID_HANDLE)
	{
		...
		if(SQL_GetRowCount(hndl) > 0)
		{
			...
			if(ccuse & CC_HASCC)
			{
				CPrintToChat(client, "%s%sA player with the name '%s%s%s' <%s%s%s> will have their custom chat privileges removed.",
					g_msg_start,
					g_msg_textcol,
					g_msg_varcol,
					sName,
			...
			{
				CPrintToChat(client, "%s%sA player with the given SteamID '%s%s%s' (name '%s%s%s') doesn't have custom chat privileges to remove.",
					g_msg_start,
					g_msg_textcol,
					g_msg_varcol,
					sAuth,
	...
	for(new client = 1; client <= MaxClients; client++)
	{
		if(IsClientInGame(client))
		{
			...
			if(StrEqual(sAuth, sAuth2))
			{
				...
				CPrintToChat(client, "%s%sYou have lost your custom chat privileges.",
					g_msg_start,
					g_msg_textcol);
			}
		}
...
DB_ShowRank(client, target, Type, Style)
{
	...
	{
		CPrintToChat(client, "%s%s%N%s is ranked %s%d%s of %s%d%s players with %s%.1f%s points.",
			g_msg_start,
			g_msg_varcol,
			target,
			g_msg_textcol,
	...
	{
		CPrintToChat(client, "%s%s%N%s is not ranked yet.",
			g_msg_start,
			g_msg_varcol,
			target,
			g_msg_textcol);
...
public DB_ShowRank_Callback(Handle:owner, Handle:hndl, String:error[], any:data)
{
	if(hndl != INVALID_HANDLE)
	{
		...
		if(SQL_FetchInt(hndl, 0) != 0)
		{
			...
			CPrintToChat(client, "%s%s%s%s is ranked %s%d%s of %s%d%s players with %s%.1f%s points.",
				g_msg_start,
				g_msg_varcol,
				sTarget,
				g_msg_textcol,
		...
		{
			CPrintToChat(client, "%s%s%s%s is not ranked yet.",
				g_msg_start,
				g_msg_varcol,
				sTarget,
				g_msg_textcol);
...
{	
	...
	{
		CPrintToChat(client, "%s%s%s %s-%s %s %sThere are no ranked players yet.",
			g_msg_start,
			g_msg_varcol,
			sType,
			g_msg_textcol,
...
DB_ShowMapsleft(client, target, Type, Style)
{
	...
	else
	{
		if(client == target)
		{
			CPrintToChat(client, "%s%sYour SteamID is not authorized. Steam servers may be down. If not, try reconnecting.",
				g_msg_start,
				g_msg_textcol);
		}
		else
		{
			...
			CPrintToChat(client, "%s%s%s's %sSteamID is not authorized. Steam servers may be down.", 
				g_msg_start,
				g_msg_varcol,
				name,
				g_msg_textcol);
	...
	if(hndl != INVALID_HANDLE)
	{
		...
		if(client != 0)
		{
			...
			else
			{
				if(clientUserId == targetUserId)
				{
					CPrintToChat(client, "%s%s%s%sYou have no maps left to beat%s%s.", 
						g_msg_start,
						g_msg_varcol,
						sType,
						g_msg_textcol,
				...
				{
					CPrintToChat(client, "%s%s has no maps left to beat%s.", 
						g_msg_start,
						g_msg_varcol,
						sType,
						sTarget,
...
DB_ShowMapsdone(client, PlayerID, Type, Style)
{
	...
	{
		CPrintToChat(client, "%s%s%s %shas no maps done.",
			g_msg_start,
			g_msg_varcol,
			sName,
			g_msg_textcol);
...
public Menu_ShowMapsdone_Callback(Handle:owner, Handle:hndl, String:error[], any:data)
{
	if(hndl != INVALID_HANDLE)
	{
		...
		else
		{
			if(client == target)
			{
				CPrintToChat(client, "%s%s%s%sYou haven't finished any maps%s%s.",
					g_msg_start,
					g_msg_varcol,
					sType,
					g_msg_textcol,
			...
			else
			{
				...
				CPrintToChat(client, "%s%s doesn't have any maps finished%s.",
					g_msg_start,
					g_msg_varcol,
					sType,
					targetname,
